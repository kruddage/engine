...
`krudd/build/build.scm` renders a `build.ninja` from the specs and drives
`ninja` directly (`native` with `cc`, `wasm` with `emcc`/`em++`); there is no
CMake in the build path.

## Pieces

```
krudd/build/build.scm              the driver: render build.ninja into build/,
                                   run ninja native|wasm (KRUDD_TARGET selects)
krudd/build/ninja/manifest.scm     the owned-directory list
krudd/build/introspect.scm         version/git facts, configure_file +
                                   changelog codegen, dependency fetch (#341, #342)
krudd/build/ninja/resolve.scm      the include/link resolver (#340)
krudd/build/ninja/ninja.scm        the build.ninja emitter (#339, #342)
krudd/build/ninja/resolve_test.scm native s7 checks + build.ninja generator
krudd/build/ninja/run-tests.sh     harness: checks, then build native + WASM
krudd/build/ninja/<dir>/build.scm  the per-directory target specs (+ their C sources)
```
...
- **include directories** — the target's own PUBLIC then PRIVATE dirs, then
  ...
  relative to the tree root (`krudd/build/ninja/`).
...
`krudd/build/introspect.scm` reads VERSION, derives the build number / commit
hash from git, and clones a pinned dependency, baking those results in at
synthesis time. The root bootstrap is free of imperative CMake, so the emitter
leans on this introspection directly. `krudd/build/run-tests.sh` covers it
(native s7 only).
...
generated by `krudd/build/introspect.scm` at synthesis time — krudd's own
`configure_file` and changelog-embed codegen (replacing CMake's) — into
`<builddir>/generated`. imgui is resolved to the `#341` fetch checkout. A
`wasm` target groups it all; `ninja wasm` (with the Emscripten toolchain on
`PATH`) builds it.
...
`krudd/build/ninja/run-tests.sh` builds and checks this when `emcc` is present.

## Retiring CMake (#343)

`krudd/build/build.scm` renders `build.ninja` into `build/` and runs
...
living beside their sources under `krudd/build/ninja/` — remain as the target
