...
kicks off an `emscripten_fetch` (see `plugins/asset/asset_plugin.c`, the
...
`modules/core/include/async_subsystem.h`) because its init — opening the
...
`subsystem_manager_get_api` (in `modules/core/subsystem_manager.c`) already
...
The asset plugin (`plugins/asset/asset_plugin.c`) is deliberately narrow: it
...
`modules/core/subsystem_manager.c`). Once called, the manager sets
...
translation unit (e.g. `plugins/backend/backend_record.c`) with no `#ifdef
