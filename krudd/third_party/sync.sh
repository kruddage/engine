...
#
# s7.c / s7.h are committed as a checksum-verified cache of the kruddage/s7
# release pinned in s7.artifact; this checks their sha256 so every build
# compiles the same bytes CI does, and re-downloads the pinned release's
# assets if a file is missing or doesn't match (e.g. after bumping the pin).
# Idempotent: once a file matches its pinned checksum, no network I/O happens
# on the next run.
#
