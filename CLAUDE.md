...
```
CMakeLists.txt    Root build (generated by krudd, gitignored — see
                  krudd/cmake/CMakeLists.scm) — emcmake cmake -S . -B build
                  drives everything
...
krudd/            The build tool — orchestrator, embedded s7 Scheme runtime,
                  and the CMakeLists.scm specs (including the root one) that
                  krudd/cmake/cmake.scm renders into the CMakeLists.txt files
                  above
...
```sh
emcmake cmake -S . -B build # configure (requires Emscripten)
