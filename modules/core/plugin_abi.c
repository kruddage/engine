...
/*
 * IndexedDB blob store for the content-addressed store's browser backing
 * (#213/#214; plugins/cas/cas_idb.c).  Separate database from
 * "krudd-project" above so its schema can evolve independently of the
 * asset-persistence bridge.  Same async contract as krudd_idb_* above:
 * cas_idb.c drives these as plain C->JS calls and learns when the cursor
 * load has resolved by polling krudd_idb_blob_state(); loaded blobs are
 * staged in a JS-side queue drained with krudd_idb_blob_peek /
 * krudd_idb_blob_pop, never delivered by JS calling back into the side
 * module by name.
 *
 * Store:  database "krudd-cas", object store "blobs" (keyPath "hash").
 * Record: { hash: string (16 lowercase hex digits), size: uint32,
 *           data: ArrayBuffer }.  The hash is rendered as a hex string on
 * the C side because a cas_hash_t is a 64-bit value and EM_JS does not
 * marshal 64-bit integers across the C/JS boundary.
 */
EM_JS(void, krudd_idb_blob_open, (void), {
	Module.__kruddIdbBlob = { state: 0, queue: [] };
	var st = Module.__kruddIdbBlob;
	try {
		var req = indexedDB.open("krudd-cas", 1);
		req.onupgradeneeded = function(ev) {
			var db = ev.target.result;
		...
		req.onsuccess = function(ev) {
			...
			var cur = tx.objectStore("blobs").openCursor();
			cur.onerror = function() { st.state = 1; };
			cur.onsuccess = function(cev) {
				var c = cev.target.result;
				if (!c) { st.state = 1; return; }
				st.queue.push(c.value);
				c.continue();
			};
		};
	} catch (e) {
...
/* 0 = loading, 1 = ready (queue populated), 2 = unavailable. */
EM_JS(int, krudd_idb_blob_state, (void), {
	return Module.__kruddIdbBlob ? Module.__kruddIdbBlob.state : 2;
})

/*
 * Inspect the front staged record without removing it: writes the
 * NUL-terminated hex hash string (truncated to hash_cap) and the byte size,
 * and returns the data byte length, or -1 when the queue is empty.
 */
EM_JS(int, krudd_idb_blob_peek, (char *hash_out, uint32_t hash_cap,
				 uint32_t *size_out), {
	var st = Module.__kruddIdbBlob;
	if (!st || st.queue.length === 0)
		return -1;
	var rec = st.queue[0];
	stringToUTF8(rec.hash || "", hash_out, hash_cap);
	HEAPU32[size_out >> 2] = rec.size >>> 0;
	var ab = rec.data;
	return ab ? ab.byteLength : 0;
})

/* Copy the front record's bytes to dst (sized via peek) and dequeue it. */
EM_JS(void, krudd_idb_blob_pop, (uint8_t *dst), {
	var st = Module.__kruddIdbBlob;
	if (!st || st.queue.length === 0)
		return;
	var rec = st.queue.shift();
...
/*
 * Insert-or-update one blob (write-behind; copies bytes into an owned
 * ArrayBuffer).  hash_ptr is a NUL-terminated hex string.
 */
EM_JS(void, krudd_idb_blob_put, (const char *hash_ptr, const void *data_ptr,
				 uint32_t size), {
	var hash = UTF8ToString(hash_ptr);
	var ab = new ArrayBuffer(size);
	if (size > 0)
		new Uint8Array(ab).set(HEAPU8.subarray(data_ptr, data_ptr + size));
	var rec = { hash: hash, size: size >>> 0, data: ab };
	try {
		var req = indexedDB.open("krudd-cas", 1);
		req.onupgradeneeded = function(ev) {
			var db = ev.target.result;
		...
		req.onsuccess = function(ev) {
			var db = ev.target.result;
			db.transaction("blobs", "readwrite")
			  .objectStore("blobs").put(rec);
		};
	} catch (e) {}
})

/* Remove the blob stored under the given hex hash (write-behind). */
EM_JS(void, krudd_idb_blob_del, (const char *hash_ptr), {
	var hash = UTF8ToString(hash_ptr);
	try {
		var req = indexedDB.open("krudd-cas", 1);
		req.onupgradeneeded = function(ev) {
...
EMSCRIPTEN_KEEPALIVE void *plugin_abi_refs[] = {
	...
	/* IndexedDB cas blob persistence (#213/#214) — backend_plugin.wasm,
	 * via plugins/cas/cas_idb.c */
	(void *)krudd_idb_blob_open,
	(void *)krudd_idb_blob_state,
	(void *)krudd_idb_blob_peek,
	(void *)krudd_idb_blob_pop,
	(void *)krudd_idb_blob_put,
	(void *)krudd_idb_blob_del,
