...
It lives in `cmake/plugins/scene_renderer/` and registers as the `scene_renderer`
...
`cmake/plugins/include/mesh.h` defines the delivery format primitives ride over the
...
`cmake/plugins/asset/primitives.c` generates cube/sphere/plane/pyramid blobs, which
