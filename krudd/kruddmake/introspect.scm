...
;;! krudd-embed-binary — like krudd-embed-file but byte-exact and length-carrying.
;;!
;;! krudd-embed-file slurps with read-char, so it only round-trips text (its
;;! bytes are the UTF-8 of a source string, and it appends a NUL terminator).
;;! A binary asset — a font, say — has arbitrary bytes 0x00..0xFF that read-char
;;! would mangle as UTF-8. This reads with read-byte and streams the C array
;;! straight to the output port (O(n), so a multi-hundred-KB font costs a linear
;;! pass, not the quadratic string-append krudd-bytes->c-init would), then emits
;;! a companion <symbol>_LEN with the exact byte count — there is no NUL
;;! terminator to lean on, and a font embeds NULs mid-array anyway.
(define (krudd-embed-binary in out symbol)
	(call-with-output-file out
	  (lambda (port)
	    (define (emit s) (write-string s port))
	    (emit (string-append
		    "/* SPDX-License-Identifier: GPL-2.0-or-later */\n"
		    "/* Generated by krudd. Do not edit. */\n"
		    "#ifndef " symbol "_H\n"
	    ...
	    (let ((n (call-with-input-file in
		       (lambda (ip)
			 (let loop ((b (read-byte ip)) (i 0))
			   (if (eof-object? b)
			       i
			       (begin
				 (emit (string-append "0x" (krudd-hex-byte b) ","))
				 (if (= 0 (modulo (+ i 1) 12)) (emit "\n\t"))
				 (loop (read-byte ip) (+ i 1)))))))))
	      (emit (string-append
		      "\n};\n\n"
		      "static const unsigned long " symbol "_LEN = "
		      (number->string n) "UL;\n\n"
		      "#endif /* " symbol "_H */\n"))))))

;;! krudd-embed-binary-optional — krudd-embed-binary when the asset exists, a
;;! zero-length stub otherwise. The UI font is provided out-of-band (see
;;! ui/kruddgui/assets/README.md); until it lands, codegen and the native build
;;! must stay green rather than erroring on a missing file, so this emits an
;;! empty blob (LEN 0). kgui_font_bake treats a zero-length blob as no font and
;;! degrades to drawing no text, logged once at runtime.
(define (krudd-embed-binary-optional in out symbol)
	(if (krudd-path-exists? in)
	    (krudd-embed-binary in out symbol)
	    (call-with-output-file out
	      (lambda (port)
		(write-string
		  (string-append
		    "/* SPDX-License-Identifier: GPL-2.0-or-later */\n"
		    "/* Generated by krudd. Do not edit. */\n"
		    "/* Placeholder: " symbol " asset was absent at build time. */\n"
		    "#ifndef " symbol "_H\n"
		    "#define " symbol "_H\n\n"
		    "static const unsigned char " symbol "[] = { 0 };\n"
		    "static const unsigned long " symbol "_LEN = 0UL;\n\n"
		    "#endif /* " symbol "_H */\n")
		  port)))))
