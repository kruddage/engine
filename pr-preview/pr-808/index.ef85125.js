...
function krudd_editor_layout_failed(reason) { var detail = 'The editor chrome could not be built, so this page has no ' + 'menus, toolbar or docks.\n\nReason: ' + UTF8ToString(reason); if (typeof window.kruddShowError === 'function') window.kruddShowError('Editor layout failed', detail); else console.error('krudd: editor layout failed — ' + UTF8ToString(reason)); }
...
function ttt_scoreboard_show() { var el = document.getElementById('ttt-scoreboard'); if (el) el.classList.remove('hidden'); }
function ttt_scoreboard_set(x,o) { var ex = document.getElementById('ttt-score-x'); var eo = document.getElementById('ttt-score-o'); if (ex) ex.textContent = x; if (eo) eo.textContent = o; }
