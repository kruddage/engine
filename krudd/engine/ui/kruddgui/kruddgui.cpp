...
static void kruddgui_tick(void)
{
	...
	/*
	 * The editor chrome: core/editor_layout.scm's menu bar, toolbar,
	 * dockspace and status fields, drawn by the image's chrome section in
	 * the immediate-mode desktop idiom. It is the one piece of the editor's
	 * panel set that is *not* parked — the parked set (kruddgui-draw) waits
	 * on the krudd-* accessors that went with kruddboard in #661, and the
	 * chrome reads nothing but the spec and the kgui-* primitives registered
	 * here, so it has nothing to wait for.
	 *
	...
	if (krudd_editor_mode())
		call_scm_panel("kruddgui-ig-tick");
