...
There is a committed bootstrap at the repo root that does all of this — installs
the toolchain, builds pinned native Dawn once (with a Wayland/X11 surface — the
~38 MB step, skipped on re-runs), and writes `.krudd-env` so `./krudd.sh editor`
finds Dawn with no manual exports. It is re-runnable: each step is skipped when
...
./krudd.sh editor   # builds and runs the native window
```
