...
function krudd_idb_open() { Module.__kruddIdb = { state: 0, queue: [] }; var st = Module.__kruddIdb; try { var req = indexedDB.open("krudd-project", 1); req.onupgradeneeded = function(ev) { var db = ev.target.result; if (!db.objectStoreNames.contains("assets")) db.createObjectStore("assets", { keyPath: "id" }); }; req.onerror = function() { st.state = 2; }; req.onsuccess = function(ev) { var db = ev.target.result; var tx = db.transaction("assets", "readonly"); var cur = tx.objectStore("assets").openCursor(); cur.onerror = function() { st.state = 1; }; cur.onsuccess = function(cev) { var c = cev.target.result; if (!c) { st.state = 1; return; } var rec = c.value; if ((rec.version >>> 0) === 1) st.queue.push(rec); c.continue(); }; }; } catch (e) { st.state = 2; } }
function krudd_idb_state() { return Module.__kruddIdb ? Module.__kruddIdb.state : 2; }
function krudd_idb_peek(id_out,type_out,path_out,path_cap) { var st = Module.__kruddIdb; if (!st || st.queue.length === 0) return -1; var rec = st.queue[0]; HEAPU32[id_out >> 2] = rec.id >>> 0; HEAP32[type_out >> 2] = rec.type | 0; stringToUTF8(rec.path || "", path_out, path_cap); var ab = rec.data; return ab ? ab.byteLength : 0; }
function krudd_idb_pop(dst) { var st = Module.__kruddIdb; if (!st || st.queue.length === 0) return; var rec = st.queue.shift(); var ab = rec.data; if (dst && ab && ab.byteLength > 0) HEAPU8.set(new Uint8Array(ab), dst); }
function krudd_idb_put(id,path_ptr,type,data_ptr,size) { var path = UTF8ToString(path_ptr); var ab = new ArrayBuffer(size); if (size > 0) new Uint8Array(ab).set(HEAPU8.subarray(data_ptr, data_ptr + size)); var rec = { version: 1, id: id >>> 0, path: path, type: type | 0, data: ab }; try { var req = indexedDB.open("krudd-project", 1); req.onupgradeneeded = function(ev) { var db = ev.target.result; if (!db.objectStoreNames.contains("assets")) db.createObjectStore("assets", { keyPath: "id" }); }; req.onsuccess = function(ev) { var db = ev.target.result; db.transaction("assets", "readwrite") .objectStore("assets").put(rec); }; } catch (e) {} }
function krudd_idb_del(id) { try { var req = indexedDB.open("krudd-project", 1); req.onupgradeneeded = function(ev) { var db = ev.target.result; if (!db.objectStoreNames.contains("assets")) db.createObjectStore("assets", { keyPath: "id" }); }; req.onsuccess = function(ev) { var db = ev.target.result; db.transaction("assets", "readwrite") .objectStore("assets").delete(id >>> 0); }; } catch (e) {} }
...
var _plugin_abi_refs = Module['_plugin_abi_refs'] = makeInvalidEarlyAccess('_plugin_abi_refs');
var _gl_abi_refs = Module['_gl_abi_refs'] = makeInvalidEarlyAccess('_gl_abi_refs');
...
function assignWasmExports(wasmExports) {
  ...
  assert(typeof wasmExports['plugin_abi_refs'] != 'undefined', 'missing Wasm export: plugin_abi_refs');
  ...
  _plugin_abi_refs = Module['_plugin_abi_refs'] = wasmExports['plugin_abi_refs'].value;
  _gl_abi_refs = Module['_gl_abi_refs'] = wasmExports['gl_abi_refs'].value;
}
