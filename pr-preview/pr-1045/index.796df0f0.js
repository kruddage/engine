...
function krudd_boot_game(out,cap) { var name = (typeof window.kruddBootGame === 'function') ? window.kruddBootGame() : ""; if (typeof name !== 'string') name = ""; stringToUTF8(name, out, cap); }
function krudd_boot_unresolved(name) { if (typeof window.kruddBootProject === 'function') window.kruddBootProject(UTF8ToString(name)); }
...
function krudd_text_input_show() { if (Module.__kruddTextEl) Module.__kruddTextEl.focus(); }
function krudd_text_input_hide() { if (Module.__kruddTextEl) Module.__kruddTextEl.blur(); }
...
function xr_js_install_state(stage,stride,max_views,in_stage,in_stride,max_inputs) { if (window.kruddXrGlue) return; var st = { session: null, space: null, failed: 0, base: stage >> 2, stride: stride, max: max_views, inBase: in_stage >> 2, inStride: in_stride, inMax: max_inputs, sel: [], near: 0.1, far: 1000.0 }; function note(line) { if (typeof console !== 'undefined') console.log('[krudd] xr: ' + line); } function context() { return (typeof GLctx !== 'undefined' && GLctx) || Module['ctx'] || null; } function fbName(fb) { if (!fb) return 0; if (!fb.name) { fb.name = GL.getNewId(GL.framebuffers); GL.framebuffers[fb.name] = fb; } return fb.name; } function selOf(src) { for (var i = 0; i < st.sel.length; i++) { if (st.sel[i].src === src) return st.sel[i]; } st.sel.push({ src: src, n: 0 }); return st.sel[st.sel.length - 1]; } function pruneSel(sources) { var keep = []; var i, j; for (i = 0; i < st.sel.length; i++) { for (j = 0; j < sources.length; j++) { if (sources[j] === st.sel[i].src) { keep.push(st.sel[i]); break; } } } st.sel = keep; } window.kruddXrGlue = { st: st, note: note, context: context, fbName: fbName, selOf: selOf, pruneSel: pruneSel }; }
function xr_js_install_input() { var g = window.kruddXrGlue; var st = g.st; if (g.stageInputs) return; g.stageInputs = function (frame) { var sources = st.session.inputSources || []; var f32 = HEAPF32; var i32 = HEAP32; var n = 0; var i; for (i = 0; i < sources.length && n < st.inMax; i++) { var src = sources[i]; var pose = (st.space && src.targetRaySpace) ? frame.getPose(src.targetRaySpace, st.space) : null; if (!pose) continue; var m = pose.transform.matrix; var e = g.selOf(src); var o = st.inBase + n * st.inStride; f32[o + 0] = pose.transform.position.x; f32[o + 1] = pose.transform.position.y; f32[o + 2] = pose.transform.position.z; f32[o + 3] = -m[8]; f32[o + 4] = -m[9]; f32[o + 5] = -m[10]; i32[o + 6] = src.handedness === 'left' ? 1 : src.handedness === 'right' ? 2 : 0; i32[o + 7] = e.n; e.n = 0; n++; } g.pruneSel(sources); return n; }; }
function xr_js_install_frame() { var g = window.kruddXrGlue; var st = g.st; if (g.onFrame) return; g.onFrame = function (time, frame) { var s = st.session; if (!s) return; s.requestAnimationFrame(g.onFrame); var layer = s.renderState.baseLayer; var pose = st.space ? frame.getViewerPose(st.space) : null; var n = 0; if (pose && layer) { var f32 = HEAPF32; var i32 = HEAP32; var views = pose.views; n = views.length < st.max ? views.length : st.max; for (var v = 0; v < n; v++) { var view = views[v]; var vp = layer.getViewport(view); var m = view.transform.inverse.matrix; var p = view.projectionMatrix; var o = st.base + v * st.stride; for (var i = 0; i < 16; i++) { f32[o + i] = m[i]; f32[o + 16 + i] = p[i]; } f32[o + 32] = view.transform.position.x; f32[o + 33] = view.transform.position.y; f32[o + 34] = view.transform.position.z; i32[o + 35] = view.eye === 'left' ? 1 : view.eye === 'right' ? 2 : 0; i32[o + 36] = vp ? vp.x : 0; i32[o + 37] = vp ? vp.y : 0; i32[o + 38] = vp ? vp.width : 0; i32[o + 39] = vp ? vp.height : 0; } } _krudd_xr_frame(g.fbName(layer && layer.framebuffer), layer ? layer.framebufferWidth : 0, layer ? layer.framebufferHeight : 0, n, g.stageInputs(frame)); _krudd_driven_tick(); }; }
function xr_js_install_session() { var g = window.kruddXrGlue; var st = g.st; if (g.probe) return; g.setup = function (session, gl) { return gl.makeXRCompatible().then(function () { var layer = new XRWebGLLayer(session, gl); session.updateRenderState({ baseLayer: layer, depthNear: st.near, depthFar: st.far }); return session.requestReferenceSpace('local-floor') .catch(function () { g.note('no local-floor; using local'); return session.requestReferenceSpace( 'local'); }); }).then(function (space) { st.space = space; _krudd_xr_session_begun(); session.requestAnimationFrame(g.onFrame); }); }; g.probe = function () { if (!navigator.xr) { _krudd_xr_report_support(1); return; } try { navigator.xr.isSessionSupported('immersive-vr') .then(function (ok) { _krudd_xr_report_support(ok ? 0 : 2); }, function () { _krudd_xr_report_support(3); }); } catch (e) { _krudd_xr_report_support(3); } }; g.request = function (depthNear, depthFar) { var gl = g.context(); if (st.session) return; if (!navigator.xr || !gl) { _krudd_xr_session_ended(1); return; } st.failed = 0; st.near = depthNear; st.far = depthFar; st.sel = []; navigator.xr.requestSession('immersive-vr') .then(function (session) { st.session = session; session.addEventListener('select', function (ev) { g.selOf(ev.inputSource).n++; }); session.addEventListener('end', function () { st.session = null; st.space = null; st.sel = []; _krudd_xr_session_ended( st.failed ? 2 : 0); }); return g.setup(session, gl); }).catch(function (e) { var s = st.session; g.note('session failed: ' + e); if (!s) { _krudd_xr_session_ended(1); return; } st.failed = 1; try { s.end(); } catch (e2) { st.session = null; st.space = null; _krudd_xr_session_ended(2); } }); }; g.end = function () { if (!st.session) return; try { st.session.end(); } catch (e) { g.note('end failed: ' + e); } }; }
function xr_js_probe() { window.kruddXrGlue.probe(); }
function xr_js_request_session(depth_near,depth_far) { window.kruddXrGlue.request(depth_near, depth_far); }
function xr_js_end_session() { window.kruddXrGlue.end(); }
function xr_js_wants_webgpu() { try { return window.kruddWantsWebGPU() ? 1 : 0; } catch (e) { return 0; } }
function project_install_bridge() { window.kruddRunProject = function (text) { var bytes = new TextEncoder().encode(String(text)); var ptr = _malloc(bytes.length + 1); if (!ptr) return -1; HEAPU8.set(bytes, ptr); HEAPU8[ptr + bytes.length] = 0; var idx = _krudd_load_project(ptr); _free(ptr); return idx; }; }
...
function webgpu_js_announce_renderer() { if (typeof window.kruddSetRenderer === 'function') window.kruddSetRenderer('webgpu'); }
function webgpu_js_announce_failed(why) { if (typeof window.kruddSetRendererFailed === 'function') window.kruddSetRendererFailed('webgpu', UTF8ToString(why)); }
function webgpu_js_status(msg) { var s = UTF8ToString(msg); if (typeof window.kruddWebGPULog === 'function') window.kruddWebGPULog(s); if (typeof console !== 'undefined') console.log('[webgpu] ' + s); }
...
var _krudd_suspend_loop = Module['_krudd_suspend_loop'] = makeInvalidEarlyAccess('_krudd_suspend_loop');
var _krudd_resume_loop = Module['_krudd_resume_loop'] = makeInvalidEarlyAccess('_krudd_resume_loop');
var _krudd_driven_tick = Module['_krudd_driven_tick'] = makeInvalidEarlyAccess('_krudd_driven_tick');
var _main = Module['_main'] = makeInvalidEarlyAccess('_main');
var _krudd_xr_probe = Module['_krudd_xr_probe'] = makeInvalidEarlyAccess('_krudd_xr_probe');
var _krudd_xr_supported = Module['_krudd_xr_supported'] = makeInvalidEarlyAccess('_krudd_xr_supported');
var _krudd_xr_request_session = Module['_krudd_xr_request_session'] = makeInvalidEarlyAccess('_krudd_xr_request_session');
var _krudd_xr_end_session = Module['_krudd_xr_end_session'] = makeInvalidEarlyAccess('_krudd_xr_end_session');
var _krudd_xr_session_active = Module['_krudd_xr_session_active'] = makeInvalidEarlyAccess('_krudd_xr_session_active');
var _krudd_xr_report_support = Module['_krudd_xr_report_support'] = makeInvalidEarlyAccess('_krudd_xr_report_support');
var _krudd_xr_session_begun = Module['_krudd_xr_session_begun'] = makeInvalidEarlyAccess('_krudd_xr_session_begun');
var _krudd_xr_session_ended = Module['_krudd_xr_session_ended'] = makeInvalidEarlyAccess('_krudd_xr_session_ended');
var _krudd_xr_frame = Module['_krudd_xr_frame'] = makeInvalidEarlyAccess('_krudd_xr_frame');
var _krudd_load_project = Module['_krudd_load_project'] = makeInvalidEarlyAccess('_krudd_load_project');
...
function assignWasmExports(wasmExports) {
  ...
  assert(typeof wasmExports['krudd_suspend_loop'] != 'undefined', 'missing Wasm export: krudd_suspend_loop');
  assert(typeof wasmExports['krudd_resume_loop'] != 'undefined', 'missing Wasm export: krudd_resume_loop');
  assert(typeof wasmExports['krudd_driven_tick'] != 'undefined', 'missing Wasm export: krudd_driven_tick');
  assert(typeof wasmExports['main'] != 'undefined', 'missing Wasm export: main');
  assert(typeof wasmExports['krudd_xr_probe'] != 'undefined', 'missing Wasm export: krudd_xr_probe');
  assert(typeof wasmExports['krudd_xr_supported'] != 'undefined', 'missing Wasm export: krudd_xr_supported');
  assert(typeof wasmExports['krudd_xr_request_session'] != 'undefined', 'missing Wasm export: krudd_xr_request_session');
  assert(typeof wasmExports['krudd_xr_end_session'] != 'undefined', 'missing Wasm export: krudd_xr_end_session');
  assert(typeof wasmExports['krudd_xr_session_active'] != 'undefined', 'missing Wasm export: krudd_xr_session_active');
  assert(typeof wasmExports['krudd_xr_report_support'] != 'undefined', 'missing Wasm export: krudd_xr_report_support');
  assert(typeof wasmExports['krudd_xr_session_begun'] != 'undefined', 'missing Wasm export: krudd_xr_session_begun');
  assert(typeof wasmExports['krudd_xr_session_ended'] != 'undefined', 'missing Wasm export: krudd_xr_session_ended');
  assert(typeof wasmExports['krudd_xr_frame'] != 'undefined', 'missing Wasm export: krudd_xr_frame');
  assert(typeof wasmExports['krudd_load_project'] != 'undefined', 'missing Wasm export: krudd_load_project');
  ...
  _krudd_suspend_loop = Module['_krudd_suspend_loop'] = createExportWrapper('krudd_suspend_loop', wasmExports['krudd_suspend_loop'], 0);
  _krudd_resume_loop = Module['_krudd_resume_loop'] = createExportWrapper('krudd_resume_loop', wasmExports['krudd_resume_loop'], 0);
  _krudd_driven_tick = Module['_krudd_driven_tick'] = createExportWrapper('krudd_driven_tick', wasmExports['krudd_driven_tick'], 0);
  _main = Module['_main'] = createExportWrapper('main', wasmExports['main'], 2);
  _krudd_xr_probe = Module['_krudd_xr_probe'] = createExportWrapper('krudd_xr_probe', wasmExports['krudd_xr_probe'], 0);
  _krudd_xr_supported = Module['_krudd_xr_supported'] = createExportWrapper('krudd_xr_supported', wasmExports['krudd_xr_supported'], 0);
  _krudd_xr_request_session = Module['_krudd_xr_request_session'] = createExportWrapper('krudd_xr_request_session', wasmExports['krudd_xr_request_session'], 0);
  _krudd_xr_end_session = Module['_krudd_xr_end_session'] = createExportWrapper('krudd_xr_end_session', wasmExports['krudd_xr_end_session'], 0);
  _krudd_xr_session_active = Module['_krudd_xr_session_active'] = createExportWrapper('krudd_xr_session_active', wasmExports['krudd_xr_session_active'], 0);
  _krudd_xr_report_support = Module['_krudd_xr_report_support'] = createExportWrapper('krudd_xr_report_support', wasmExports['krudd_xr_report_support'], 1);
  _krudd_xr_session_begun = Module['_krudd_xr_session_begun'] = createExportWrapper('krudd_xr_session_begun', wasmExports['krudd_xr_session_begun'], 0);
  _krudd_xr_session_ended = Module['_krudd_xr_session_ended'] = createExportWrapper('krudd_xr_session_ended', wasmExports['krudd_xr_session_ended'], 1);
  _krudd_xr_frame = Module['_krudd_xr_frame'] = createExportWrapper('krudd_xr_frame', wasmExports['krudd_xr_frame'], 5);
  _krudd_load_project = Module['_krudd_load_project'] = createExportWrapper('krudd_load_project', wasmExports['krudd_load_project'], 1);
