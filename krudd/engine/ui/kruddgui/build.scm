...
((embed "kruddgui.scm" "kruddgui_scm.h" "KRUDDGUI_SCM")
 ...
 (native-only
  ...
  ;;! chrome section. Like the panel tests above it stubs the kgui-* primitives
  ;;! and loads the real image; unlike them it needs no fixture for what it
  ;;! draws, because the spec rides in the linked script library already.
  (executable "kgui_chrome_test"
              (sources "kgui_chrome_test.c")
              (private (root "core/include") (raw "${generated}")
                       (raw "../third_party"))
              (link "script"))
  (test "kgui_chrome" "kgui_chrome_test")))
