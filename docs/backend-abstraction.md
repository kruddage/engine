...
kicks off an `emscripten_fetch` (see `cmake/plugins/asset/asset_plugin.c`, the
...
`cmake/modules/core/include/async_subsystem.h`) because its init — opening the
...
`subsystem_manager_get_api` (in `cmake/modules/core/subsystem_manager.c`) already
...
### `backend_api` struct (as implemented in `cmake/plugins/include/backend_api.h`)
...
The asset plugin (`cmake/plugins/asset/asset_plugin.c`) is deliberately narrow: it
...
`cmake/modules/core/subsystem_manager.c`). Once called, the manager sets
...
translation unit (e.g. `cmake/plugins/backend/backend_record.c`) with no `#ifdef
